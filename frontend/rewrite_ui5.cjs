          ...
          <div className="space-y-6 flex flex-col h-full animate-fadeIn transition-all">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col items-center justify-center relative overflow-hidden group hover:border-indigo-500/50 transition">
                ...
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-1">Avg Response</span>
              </div>
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl flex flex-col items-center justify-center relative overflow-hidden group hover:border-emerald-500/50 transition">
                ...
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-1">Cache Efficiency</span>
              </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl flex flex-col shadow-lg">
                <h3 className="text-sm font-semibold text-slate-400 mb-4 uppercase tracking-wider flex items-center justify-between">
                  <span className="flex items-center gap-2"><BarChart2 className="w-4 h-4 text-blue-400" /> Query Volume Trend (QPS)</span>
                ...
                <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={stats?.history_series || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip contentStyle={{backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '8px', color: '#f8fafc', fontSize: '13px'}} cursor={{stroke: '#334155', strokeWidth: 1, strokeDasharray: '4 4'}} />
                ...
                <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={stats?.history_series || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip contentStyle={{backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '8px', color: '#f8fafc', fontSize: '13px'}} cursor={{stroke: '#334155', strokeWidth: 1, strokeDasharray: '4 4'}}/>
                      <Area yAxisId="left" type="monotone" name="Latency" dataKey="latency" stroke="#818cf8" strokeWidth={2} fillOpacity={1} fill="url(#colorLat)" isAnimationActive={false} />
                  ...
                  {stats?.telemetry_alerts?.length > 0 ? stats.telemetry_alerts.map((a, i) => (
                    <div key={i} className={\`p-3 rounded-lg border text-xs leading-relaxed \${a.Level === 'CRIT' ? 'bg-rose-500/10 border-rose-500/30 text-rose-200' : 'bg-amber-500/10 border-amber-500/30 text-amber-200'}\`}>
                       <div className="mb-1 flex justify-between items-center">
                          <span className={\`font-bold \${a.Level === 'CRIT' ? 'text-rose-400' : 'text-amber-400'}\`}>[{a.Level}] ANOMALY ALARM</span>
                          <span className="opacity-70 font-mono text-[10px]">{a.Time}</span>
                       </div>
                       <p className="opacity-90">{a.Message}</p>
                    </div>
                  )) : (
                    <div className="flex h-[150px] flex-col items-center justify-center text-slate-500 space-y-2 opacity-50">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
             ...
             <div className="flex flex-col gap-6">
               <div className="bg-[#0f172a] border border-slate-800 p-6 rounded-xl shadow-lg">
                 ...
                 <div className="space-y-3">
                   {topAnalytics.clients.map((c, i) => (
                     <div key={i} className="flex justify-between items-center px-4 py-2.5 bg-[#0b1120] rounded-lg border border-slate-800/50 hover:bg-slate-800/20">
                       <span className="text-blue-300 text-sm font-mono flex items-center gap-3"><span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span>{c.name}</span>
                       <div className="flex gap-4 text-[11px] font-bold tracking-wider">
                         <span className="text-emerald-400">{c.allow ? c.allow.toLocaleString() : 0} OK</span>
                         <span className="text-rose-400 border-l border-slate-700 pl-4">{c.block ? c.block.toLocaleString() : 0} DROP</span>
                       </div>
                     </div>
                   ))}
               ...
               <div className="bg-[#0f172a] border border-slate-800 p-6 rounded-xl shadow-lg">
                 <h3 className="text-sm font-semibold text-slate-400 mb-4 uppercase tracking-wider text-rose-400">Top Access Blocked (Global Lists)</h3>
                 <div className="space-y-3">
                   {topAnalytics.blocked.map((d, i) => (
                     <div key={i} className="flex justify-between items-center px-4 py-2 bg-[#0b1120] rounded-lg border border-rose-900/30 hover:border-rose-800/50">
                       <span className="text-rose-100 text-sm truncate pr-2 font-mono flex items-center gap-3"><span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span>{d.name}</span>
